...
- `auto` now resolves to `winfsp` when the current `filmuvfs.exe` build includes the WinFSP backend, and falls back to `projfs` otherwise.
...
- `winfsp` (recommended)
- `auto` (prefers `winfsp`, falls back to `projfs` if the WinFSP backend is unavailable)
