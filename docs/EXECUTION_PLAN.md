...
- route decomposition progress note: shared runtime-governance extraction from `stream.py` into `runtime_governance.py` and remote-HLS/HLS-failure governance extraction into `runtime_hls_governance.py` are now landed as no-behavior-change boundary steps; remaining large-file decomposition still targets deeper `stream.py` segmentation plus `media.py`, `playback.py`, and `workers/tasks.py`.
