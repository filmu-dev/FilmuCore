...
- route/service decomposition progress note: shared runtime-governance extraction from `stream.py` into `runtime_governance.py`, remote-HLS/HLS-failure governance extraction into `runtime_hls_governance.py`, route refresh-trigger plus refresh-dispatch policy extraction into `runtime_refresh_governance.py`, playback refresh-deferral governance extraction from `playback.py` into `playback_deferral_governance.py`, and worker-stage observability plus stable stage/job-id helper extraction from `workers/tasks.py` into `stage_observability.py` and `stage_job_ids.py` are now landed as no-behavior-change boundary steps; remaining large-file decomposition still targets deeper `stream.py` segmentation plus `media.py`, `playback.py`, and `workers/tasks.py`.
