...
- route/service decomposition progress note: shared runtime-governance extraction from `stream.py` into `runtime_governance.py`, remote-HLS/HLS-failure governance extraction into `runtime_hls_governance.py`, route refresh-trigger plus refresh-dispatch policy extraction into `runtime_refresh_governance.py`, and status-payload extraction into `runtime_status_payload.py` are now landed as no-behavior-change route boundaries; service/worker boundaries now also include `playback_deferral_governance.py`, `playback_refresh_dispatch.py`, `media_path_inference.py`, `stage_observability.py`, `stage_job_ids.py`, and `stage_isolation.py`, with decomposition size budgets/contract tests enforced in CI (`tests/test_*_module_boundaries.py`) and continuity posture enforced by [`../scripts/check_enterprise_rollout_continuity.ps1`](../scripts/check_enterprise_rollout_continuity.ps1).
