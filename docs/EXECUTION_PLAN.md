...
- route decomposition progress note: shared runtime-governance extraction from `stream.py` into `runtime_governance.py` is now landed as a no-behavior-change boundary step; remaining large-file decomposition still targets `media.py`, `playback.py`, and `workers/tasks.py`.
