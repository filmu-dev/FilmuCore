...
- Director-facing operator posture read models, including control-plane recovery, replay-backplane proof, observability pipeline proof, and plugin readiness
...
3. Replace GraphQL dependencies on compatibility route helpers with shared service builders.
4. Keep REST stable only where the old frontend still depends on it.
5. Remove compatibility REST routes once they have no active frontend consumer.
