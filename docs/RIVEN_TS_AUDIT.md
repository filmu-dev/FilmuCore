...
- graph-first specialization-backed calendar/detail/list lineage on the shared media-domain service seam, plus specialization-backed mounted catalog path shaping on the FilmuVFS supplier boundary, specialization-backed REST detail season coverage, and specialization-normalized compatibility metadata
...
- broader compatibility-tail read-model adoption beyond the now-landed graph/VFS/detail-first specialization projections and compatibility-metadata normalization
