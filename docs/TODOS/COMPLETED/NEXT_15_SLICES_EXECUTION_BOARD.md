...
- `Slice G` is implemented: the enterprise `vfs_data_plane` governance slice consumes the same live runtime rollout posture already exposed by `/api/v1/stream/status`, including runtime-snapshot availability, rollout readiness, rollout reasons, cache/fallback/prefetch ratios, provider/fairness pressure incidents, and explicit mounted cache/chunk-coalescing/upstream-wait/refresh pressure classes with machine-readable reasons.
...
- `Post-board Slice Z` is implemented: mounted/VFS runtime rollups now classify cache pressure, chunk-coalescing pressure, upstream wait pressure, and refresh pressure directly on `/api/v1/stream/status` and `/api/v1/operations/governance`, with bounded machine-readable reason lists instead of leaving operators to infer those states from raw counters.
- `Post-board Slice AA` is implemented: the Windows soak artifact path now records the same mounted cache/chunk-coalescing/upstream-wait/refresh pressure classes and reason lists inside `summary.json`, captures the backend API’s pressure view alongside the runtime delta, and emits explicit threshold failures when any runtime pressure class becomes `critical`.
- `Post-board Slice AB` is implemented: multi-environment VFS soak aggregation is now a first-class repo gate through `run_windows_vfs_multi_environment_gate.ps1`, which scans repeated `soak-stability-*.json` artifacts across distinct `environment_class` values, enforces minimum environment count plus required profile coverage, and fails when any environment reports critical mounted pressure runs.
- `Post-board Slice AC` is implemented: heavy-stage isolation now has a stricter enterprise policy baseline through spawn-required process pools, bounded worker-ceiling enforcement, recycle-budget validation, and explicit policy-violation visibility on `/api/v1/stream/status` plus `/api/v1/operations/governance`.
- `Post-board Slice AD` is implemented: specialization-backed media read models now eager-load `Movie`/`Show`/`Season`/`Episode` hierarchy in the shared service layer, GraphQL `mediaItem` now exposes lineage fields directly from that projection, and calendar/detail projections no longer fall back to metadata-first hierarchy shaping.
- `Post-board Slice AE` is implemented: GraphQL `items` now runs through `MediaService.search_items()` so graph list consumers reuse the specialization-backed summary projection, and `GQLMediaItem` now carries additive lineage/presentation fields instead of stopping at generic media type/kind only.
- `Post-board Slice AF` is implemented: the FilmuVFS catalog supplier now reuses that same specialization hierarchy for mounted path shaping, so media type, show title, and season/episode lineage prefer persisted specialization rows before metadata or filename/provider-path inference.
- `Post-board Slice AG` is implemented: REST item-detail `covered_season_numbers` now prefers persisted `Show -> Season` specialization rows before falling back to media-entry path inference, so compatibility `seasonStates` stop drifting on stale provider-pack paths.
- `Post-board Slice AH` is implemented: extended compatibility `metadata` payloads now normalize `item_type`, identifiers, parent ids, show title, and season/episode fields from the specialization record so legacy nested-key consumers no longer see stale hierarchy values that disagree with the typed projections.
- `Post-board Slice AI` is implemented: the keyed `/api/v1/calendar` compatibility payload now carries additive `imdb_id` plus parent ids from the specialization-backed calendar projection, so legacy calendar consumers no longer lose identity context that already exists on the shared domain seam.
- `Post-board Slice AJ` is implemented: GraphQL `calendarEntries` now exposes additive `imdbId` plus parent TMDB/TVDB ids from the specialization-backed calendar projection, keeping the graph surface aligned with the shared domain seam instead of dropping identity context at the boundary.
- `Post-board Slice AK` is implemented: GraphQL now exposes `vfsDirectory` and `vfsCatalogEntry` directly from the shared `vfs_catalog_supplier` snapshot, so graph-first control-plane consumers can browse mounted directory/stat state, lease/control fields, and correlation keys without widening REST.
- `Post-board Slice AL` is implemented: GraphQL now exposes `vfsSnapshot`, so graph-first control-plane consumers can read generation, publish time, blocked counts, and aggregate mounted snapshot posture from the shared catalog supplier.
- `Post-board Slice AM` is implemented: GraphQL now exposes `vfsBlockedItems`, so blocked mounted rows are visible on the graph surface without falling back to REST or sidecar-only diagnostics.
- `Post-board Slice AN` is implemented: GraphQL now exposes `runtimeLifecycle` directly from the bounded in-memory lifecycle graph, so bootstrap/degraded/steady-state transitions are visible to graph-first control-plane consumers.
- `Post-board Slice AO` is implemented: GraphQL now exposes `workerQueueStatus` from the shared Redis-backed queue reader, including queue depth, retry/result pressure, dead-letter counts, and alert state.
- `Post-board Slice AP` is implemented: GraphQL now exposes filtered `workerQueueHistory`, so graph-first operator views can consume bounded queue trends and replay-oriented dead-letter filters without widening REST.
- `Post-board Slice AQ` is implemented: GraphQL now exposes `workerMetadataReindexStatus` from the shared Redis-backed metadata reindex status store, so the latest reconciliation outcome and repair counters are graph-visible.
- `Post-board Slice AR` is implemented: GraphQL now exposes `workerMetadataReindexHistory`, so bounded reconciliation trends are available to graph-first operator consumers without duplicating route-local models.
- `Post-board Slice AS` is implemented: `mediaItem` now exposes persisted `playbackAttachments`, so graph item detail carries attachment-level provider and refresh-state context from the shared detail seam.
- `Post-board Slice AT` is implemented: `mediaItem` now exposes `resolvedPlayback`, so graph item detail surfaces current direct/HLS readiness from the shared playback snapshot seam.
- `Post-board Slice AU` is implemented: `mediaItem` now exposes `activeStream` ownership plus `mediaEntries`, so graph item detail finally carries persisted media-entry ownership and active-role mapping instead of leaving that detail to compatibility-only consumers.
- `Post-board Slice AV` is implemented: GraphQL now exposes `triggerDirectPlaybackRefresh`, backed directly by the shared app-scoped playback refresh controllers so graph-first control-plane consumers can request direct-play refresh work without widening REST.
- `Post-board Slice AW` is implemented: GraphQL now exposes `triggerHlsFailedLeaseRefresh`, backed directly by the shared selected-HLS failed-lease refresh controllers with controller outcome and nested refresh result visibility.
- `Post-board Slice AX` is implemented: GraphQL now exposes `triggerHlsRestrictedFallbackRefresh`, backed directly by the shared selected-HLS restricted-fallback refresh controllers with deferred-retry and execution-detail visibility.
- `Post-board Slice AY` is implemented: GraphQL now exposes `markSelectedHlsMediaEntryStale`, backed directly by the shared playback service seam so graph-first control-plane consumers can request HLS stale marking without widening REST.
- `Post-board Slice AZ` is implemented: GraphQL now exposes `persistMediaEntryControlState`, backed directly by the shared playback-service write seam so graph-first control-plane consumers can persist bounded media-entry URL/state corrections plus active-role rebinding without widening REST or pushing DB mutation into resolvers.
- `Post-board Slice BA` is implemented: GraphQL now exposes `persistPlaybackAttachmentControlState`, backed directly by the shared playback-service write seam so graph-first control-plane consumers can persist bounded attachment URL/state corrections while synchronizing linked media-entry rows instead of creating split-brain playback state across the persisted layers.
- Execution bias from this point is explicit: new read-model and control-plane breadth should land graph-first, with REST only receiving the minimum compatibility-preserving additive changes required by the current frontend.
