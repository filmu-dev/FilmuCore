...
- The next Priority 2 gap after this slice is broader read-model adoption across the remaining compatibility consumers plus mutation-side control-plane depth; graph, VFS, the key REST detail season-coverage seam, extended compatibility metadata, additive calendar identity fields, GraphQL VFS directory/stat/snapshot consumers, and graph playback/media-entry ownership consumers are now landed on specialization-backed reads.
