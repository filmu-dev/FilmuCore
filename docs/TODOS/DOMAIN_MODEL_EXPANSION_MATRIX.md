...
- The next Priority 2 gap after this slice is broader read-model adoption across the remaining compatibility consumers plus broader persisted mutation/control-plane depth; graph, VFS, the key REST detail season-coverage seam, extended compatibility metadata, additive calendar identity fields, GraphQL VFS directory/stat/snapshot consumers, graph playback/media-entry ownership consumers, graph playback refresh/stale-mark mutations, and the persisted graph media-entry control-state mutation are now landed on specialization-backed reads.
