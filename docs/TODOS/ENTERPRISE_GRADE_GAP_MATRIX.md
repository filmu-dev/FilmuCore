...
| Plugin trust and isolation | [`filmu_py/plugins/manifest.py`](../../filmu_py/plugins/manifest.py) and [`filmu_py/plugins/loader.py`](../../filmu_py/plugins/loader.py) now provide manifest, host-version, release-channel, trust-level, and permission-scope validation | Add plugin signing, provenance, runtime sandboxing, quarantine/revocation, and stronger external-author distribution controls | Plugin extensibility becomes a liability without a trust model | P1 |
