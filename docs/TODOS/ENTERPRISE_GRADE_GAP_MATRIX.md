...
| Distributed control plane | [`filmu_py/core/event_bus.py`](../../filmu_py/core/event_bus.py) and [`filmu_py/core/log_stream.py`](../../filmu_py/core/log_stream.py) are process-local/in-memory, and [`GET /api/v1/operations/governance`](../../filmu_py/api/routes/default.py) now exposes that non-HA posture explicitly | Add cross-node eventing, replayable streams, subscription durability, node coordination, failover semantics, and HA readiness | Single-process signaling is a ceiling on scale, availability, and operator confidence | P0 |
| SRE / production operations program | Strong local proof scripts exist, [`OPERATIONS_PROGRAM.md`](../OPERATIONS_PROGRAM.md) now defines SLO/error-budget, rollback, backup/restore, incident, canary/rollout, and capacity policy, and [`GET /api/v1/operations/governance`](../../filmu_py/api/routes/default.py) exposes SRE posture | Exercise and automate SLO enforcement, rollback, backup/restore, DR proof, incident runbooks, canary/rollout policy, and capacity review cadence | Enterprise-grade systems are judged by operability under failure, not only by feature breadth | P0 |
| Durable operator log pipeline | [`filmu_py/logging.py`](../../filmu_py/logging.py) now provides rotating ECS/NDJSON-style file output plus the bounded in-memory broker in [`filmu_py/core/log_stream.py`](../../filmu_py/core/log_stream.py), `/api/v1/workers/queue/history` now exposes bounded queue-history summaries instead of raw samples only, [`OPERATOR_LOG_PIPELINE.md`](../OPERATOR_LOG_PIPELINE.md) defines shipper/search/replay taxonomy expectations, and [`GET /api/v1/operations/governance`](../../filmu_py/api/routes/default.py) exposes log-pipeline posture | Add actual shipper/search workflow, broader trace/span adoption, and documented export/search operations comparable to the current `riven-ts` ECS + Filebeat + Elastic stack | Enterprise operations require searchable durable logs, not only live process-local compatibility streams | P0 |
