...
| --- | --- | --- | --- | --- |
| Enterprise identity and authz | [`filmu_py/api/deps.py`](../../filmu_py/api/deps.py) now carries additive actor/tenant/role/scope request context with no implicit admin fallback, computes `effective_permissions`, [`filmu_py/services/identity.py`](../../filmu_py/services/identity.py) persists tenant/principal/service-account inventory, actor-aware API-key rotation now also rotates `api_key_id`, and [`GET /api/v1/auth/context`](../../filmu_py/api/routes/default.py) exposes the resolved actor mapping and effective permissions, but authentication is still anchored on one shared API key | Add OIDC/SSO readiness, deeper RBAC/ABAC policy enforcement, tenant-scoped authorization, and operator-visible access policy state | Enterprise-grade control plane requires traceable identity and policy, not one shared backend secret plus additive headers | P0 |
| Tenancy model | [`TenantORM`](../../filmu_py/db/models.py) now exists, `tenant_id` ownership covers the main `media_items` and `item_requests` lifecycle carriers, tenant-scoped reads now reach item/calendar/stats routes, worker context binds tenant ownership, and plugin load visibility now exposes tenancy posture, but quotas, plugin execution, VFS governance, and metrics ownership are still not fully tenant-aware | Define tenant boundary, tenant-aware quota policy, plugin/VFS/control-plane attribution, and tenant-scoped observability | Surpassing `riven-ts` means Filmu can safely scale beyond single-operator deployment assumptions | P0 |
...
1. Continue the new effective-permission, actor-aware key-rotation, and tenant-persistence baseline into a real enterprise identity/tenancy/authz platform rather than treating the current header-carried context as the end state.
