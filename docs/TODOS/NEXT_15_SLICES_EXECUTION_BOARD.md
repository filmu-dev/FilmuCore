...
- `Wave 3` is now implementation-complete in local `main`: the repo now encodes the lifecycle graph, dedicated index stage, DLQ/idempotency taxonomy, isolated heavy-stage budgets, and optional queued stream-link refresh path. Remaining work is environment soak/proof and stricter sandbox ceilings, not missing orchestration primitives.
