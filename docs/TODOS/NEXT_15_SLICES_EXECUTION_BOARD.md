...
- `Slice K` backend baseline is implemented: access-policy revisions now carry route-context permission constraints, `/api/v1/auth/policy` exposes constrained authorization probes, and `require_permissions()` records route-aware policy decisions instead of relying on shallow permission-string checks alone.
- `Slice L` backend baseline is implemented: `/api/v1/auth/policy` and `/api/v1/operations/governance` now expose `oidc_rollout_status`, configuration completeness, and API-key-fallback posture so OIDC is rollout-diagnosable rather than just enabled/disabled.
- `Slice M` backend baseline is implemented: authorization decisions now persist into `authorization_decision_audit`, `/api/v1/auth/policy/audit` supports bounded actor/tenant/permission/reason/path search, and repeated-denial alert candidates are surfaced from the result set.
...
- `Wave 2` is now backend-complete in local `main`: the remaining requirement is environment rollout proof, broader resource-ownership ABAC depth, and production alerting discipline before treating slices `K` through `M` as fully exit-gated.
