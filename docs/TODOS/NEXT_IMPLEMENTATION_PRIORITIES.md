...
- The operator APIs now expose more of that roadmap directly: [`GET /api/v1/operations/governance`](../../filmu_py/api/routes/default.py) now reports a 10-slice enterprise posture contract, [`GET /api/v1/auth/policy/revisions`](../../filmu_py/api/routes/default.py) plus write/approve/reject/activate routes expose persisted access-policy approval workflows, [`GET /api/v1/plugins/governance/overrides`](../../filmu_py/api/routes/default.py) exposes persisted plugin quarantine/revocation overrides, and [`GET /api/v1/operations/control-plane/subscribers`](../../filmu_py/api/routes/default.py) exposes durable replay subscriber ownership/offset state.
...
4. keep the newly explicit Docker Plex proof checks green in repeated runs: host-binary freshness, entry-id refresh-identity visibility, foreground-fetch/coalescing visibility in the proof artifact bundle, and the new playback-proof enterprise wrapper now emits `environment_class` plus threshold policy in its summary JSON so merge-policy promotion can reason from artifacts instead of free-form notes
...
- Mounted/VFS operator visibility is also no longer split between the Python bridge and sidecar-local artifacts: `/api/v1/stream/status` now reports FilmuVFS catalog watch-session churn, reconnect-delta behavior, current-generation reconnect reuse, inline-refresh deduplication, supplier failure classes, provider-backed refresh outcomes, explicit inline refresh request outcomes from the gRPC bridge, additive `vfs_runtime_*` counters ingested from the Rust sidecar snapshot, and active `session_id`/`handle_key` summaries, so the next observability step is failure-taxonomy refinement and repeated soak evidence rather than first cross-process status exposure.
...
- deepen the now-landed trust-store/publisher-policy baseline into stronger runtime sandboxing, artifact provenance/distribution policy, and operator quarantine/revocation workflows above the newly landed publisher lifecycle/distribution/scope controls
...
- shipper/search workflow above the now-landed durable structured logs; the repo now also contains a local Vector plus OpenSearch reference stack under [`../../ops/log-pipeline`](../../ops/log-pipeline), so the remaining work is environment promotion and alert integration
...
- broader resource-level ABAC across more control-plane actions now that persisted access-policy approval/audit-search workflows exist
- richer tenant quota enforcement across provider/playback/plugin/VFS paths and broader tenancy propagation across plugin execution, VFS governance, metrics ownership, and control-plane attribution
