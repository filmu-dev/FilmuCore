...
Turn Priority 3 from [`NEXT_IMPLEMENTATION_PRIORITIES.md`](NEXT_IMPLEMENTATION_PRIORITIES.md) into an executable planning artifact.
...
- It is still intentionally below a full dedicated `parse-scrape-results` worker stage: scrape fan-out, structured parser settings, richer validation, and standalone parse-stage observability are still missing.
