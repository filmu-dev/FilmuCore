...
- [ ] keep `FILMU_PLAYBACK_STABILITY_HISTORY_ROOT`, `FILMU_SOAK_HISTORY_ROOT`, and `FILMU_LOG_PIPELINE_HISTORY_ROOT` environment roots configured so trend evidence remains continuous across runner lifecycles
...
- [x] live protected-branch policy validation is now enforced in CI via admin-authenticated [`../../scripts/check_github_main_policy.ps1`](../../scripts/check_github_main_policy.ps1) `-ValidateCurrent` and fails the gate when branch settings drift
- [x] playback stability trend-regression governance now exists via [`../../scripts/check_playback_stability_trends.ps1`](../../scripts/check_playback_stability_trends.ps1) and is wired into [`../../run_playback_gate_ci.sh`](../../run_playback_gate_ci.sh)
- [x] enterprise continuity governance now exists via [`../../scripts/check_enterprise_rollout_continuity.ps1`](../../scripts/check_enterprise_rollout_continuity.ps1) and [`.github/workflows/enterprise-rollout-continuity.yml`](../../.github/workflows/enterprise-rollout-continuity.yml), combining decomposition, soak, playback, and operator-evidence posture in one gate
...
2. keep the CI-enforced live `main` branch-policy validation (`-ValidateCurrent`) green using [`../../scripts/check_github_main_policy.ps1`](../../scripts/check_github_main_policy.ps1) and [`../PLAYBACK_GATE_RUNNER_SETUP.md`](../PLAYBACK_GATE_RUNNER_SETUP.md)
...
- playback-gate CI now also runs live admin-authenticated branch-policy validation as a required gate (`check_github_main_policy.ps1 -ValidateCurrent`), with the current-policy artifact retained as evidence but not treated as sufficient on its own
- Windows VFS soak promotion now also has a scheduled multi-environment program and trend-regression gate via [`../../scripts/run_windows_vfs_soak_program.ps1`](../../scripts/run_windows_vfs_soak_program.ps1) and [`../../scripts/check_windows_vfs_soak_trends.ps1`](../../scripts/check_windows_vfs_soak_trends.ps1)
...
- Windows-native soak proof is now more decision-shaped too: [`../../scripts/run_windows_vfs_soak.ps1`](../../scripts/run_windows_vfs_soak.ps1) emits threshold checks and named diagnostics for reconnect churn, stale-refresh failures, cache cold-fetch churn, provider-pressure mentions, mounted-read fatal classifications, and now also mounted cache/chunk-coalescing/upstream-wait/refresh pressure classes directly into the existing artifact family rather than leaving the soak rubric as prose-only guidance
...
- that same native Windows evidence path now also derives `cache_pressure_class`, `chunk_coalescing_pressure_class`, `upstream_wait_class`, and `refresh_pressure_class` with bounded reason lists from `filmuvfs-runtime-status.json`, records the backend API’s matching pressure view, and fails the soak run when any runtime pressure class becomes `critical`
...
- multi-environment Windows VFS evidence is now also a first-class repo gate instead of a manual afterthought: [`../../scripts/run_windows_vfs_multi_environment_gate.ps1`](../../scripts/run_windows_vfs_multi_environment_gate.ps1) aggregates `soak-stability-*.json` artifacts across distinct `environment_class` values, enforces minimum environment count and required profile coverage, and fails if any environment reports critical mounted pressure runs or missing required runtime/backend capture
- the remaining playback-proof gap is no longer first preferred-client playback, first native Windows Emby proof, first Docker Plex playback-start evidence, native Plex targeting, first route-to-controller handoff for failed inline remote-HLS repair, first selected-HLS provider-pressure deferral visibility, first widening of those same semantics into direct background refresh work, first repeated provider-parity gate coverage, or first live-policy gate validation. It is keeping those required gates green across longer-running multi-environment soak windows and continuing operations hardening.
