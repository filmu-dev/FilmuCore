...
- persisted operator overrides now sit above manifest defaults through [`../../filmu_py/services/plugin_governance.py`](../../filmu_py/services/plugin_governance.py), and operators can manage approved/quarantined/revoked state through `GET /api/v1/plugins/governance`, `GET /api/v1/plugins/governance/overrides`, and `PUT /api/v1/plugins/governance/{plugin_name}`.
