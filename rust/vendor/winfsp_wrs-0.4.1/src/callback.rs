...
use crate::{
    ...
    FileContextMode, FileInfo, OpenFileInfo, PSecurityDescriptor, SecurityDescriptor, VolumeInfo,
    WriteMode,
};

#[cfg(feature = "debug")]
fn append_wrapper_trace(line: &str) {
    let path = std::env::var("FILMUVFS_WINFSP_WRAPPER_LOG").unwrap_or_else(|_| {
        std::env::temp_dir()
            .join("filmuvfs-winfsp-wrapper.log")
            .display()
            .to_string()
    });
...
impl TrampolineInterface {
    ...
    unsafe extern "C" fn get_security_by_name_ext<C: FileSystemInterface>(
        ...
        eprintln!(
            "winfsp_wrs.get_security_by_name_ext enter file_name_ptr={file_name:p} security_descriptor_ptr={security_descriptor:p} size_ptr={p_security_descriptor_size:p}"
        );
        append_wrapper_trace("get_security_by_name_ext.enter");
        let fs = &*(*file_system).UserContext.cast::<C>();
    ...
    unsafe extern "C" fn open_ext<C: FileSystemInterface>(
        ...
        eprintln!(
            "winfsp_wrs.open_ext enter file_name_ptr={file_name:p} create_options=0x{create_options:08X} granted_access=0x{granted_access:08X}"
        );
        append_wrapper_trace("open_ext.enter");
        let fs = &*(*file_system).UserContext.cast::<C>();
        let file_name = U16CStr::from_ptr_str(file_name);
        let open_file_info = unsafe {
            file_info
                .cast::<OpenFileInfo>()
                .as_mut()
                .expect("FSP_FSCTL_OPEN_FILE_INFO buffer was not allocated.")
        };
    ...
    unsafe extern "C" fn create_ext<C: FileSystemInterface>(
        ...
        eprintln!(
            "winfsp_wrs.create_ext enter file_name_ptr={file_name:p} create_options=0x{create_options:08X} granted_access=0x{granted_access:08X} file_attributes=0x{file_attributes:08X} security_descriptor_ptr={security_descriptor:p} allocation_size={allocation_size}"
        );
        append_wrapper_trace("create_ext.enter");
        let fs = &*(*file_system).UserContext.cast::<C>();
        let file_name = U16CStr::from_ptr_str(file_name);
        let open_file_info = unsafe {
            file_info
        ...
        let sd = if security_descriptor.is_null() {
            eprintln!("winfsp_wrs.create_ext null security descriptor");
            SecurityDescriptor::default()
        } else {
            SecurityDescriptor::from_ptr(security_descriptor)
        };
    ...
    unsafe extern "C" fn create_ex_ext<C: FileSystemInterface>(
        ...
        eprintln!(
            "winfsp_wrs.create_ex_ext enter file_name_ptr={file_name:p} create_options=0x{create_options:08X} granted_access=0x{granted_access:08X} file_attributes=0x{file_attributes:08X} security_descriptor_ptr={security_descriptor:p} allocation_size={allocation_size} extra_length={extra_length} reparse={extra_buffer_is_reparse_point}"
        );
        let fs = &*(*file_system).UserContext.cast::<C>();
        let file_name = U16CStr::from_ptr_str(file_name);
        let open_file_info = unsafe {
        ...
        let sd = if security_descriptor.is_null() {
            eprintln!("winfsp_wrs.create_ex_ext null security descriptor");
            SecurityDescriptor::default()
        } else {
            SecurityDescriptor::from_ptr(security_descriptor)
        };
    ...
    pub(crate) fn interface<Ctx: FileSystemInterface>() -> FSP_FILE_SYSTEM_INTERFACE {
        ...
        FSP_FILE_SYSTEM_INTERFACE {
            ...
            ..Default::default() // Initializing `Reserved` fields
        }
    }
}
