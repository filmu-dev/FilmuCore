...
impl FileSystem {
    ...
    pub fn start<Ctx: FileSystemInterface>(
        ...
        let interface_value = TrampolineInterface::interface::<Ctx>();

        #[cfg(feature = "debug")]
        eprintln!(
            "winfsp_wrs.interface create_defined={} create_ex_defined={} open_defined={} get_security_by_name_defined={} read_directory_defined={} file_context_mode={:?} mountpoint={}",
            interface_value.Create.is_some(),
            interface_value.CreateEx.is_some(),
            interface_value.Open.is_some(),
            interface_value.GetSecurityByName.is_some(),
            interface_value.ReadDirectory.is_some(),
            Ctx::FileContext::MODE,
            mountpoint
                .map(|value| value.to_string_lossy())
                .unwrap_or_else(|| "<auto>".to_owned())
        );

        let interface = Box::into_raw(Box::new(interface_value));
