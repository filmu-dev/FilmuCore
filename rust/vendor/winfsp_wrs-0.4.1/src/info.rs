...
pub struct OpenFileInfo(pub(crate) FSP_FSCTL_OPEN_FILE_INFO);

impl AsRef<FileInfo> for OpenFileInfo {
    fn as_ref(&self) -> &FileInfo {
        unsafe { &*(std::ptr::addr_of!(self.0.FileInfo).cast::<FileInfo>()) }
    }
}

impl AsMut<FileInfo> for OpenFileInfo {
    fn as_mut(&mut self) -> &mut FileInfo {
        unsafe { &mut *(std::ptr::addr_of_mut!(self.0.FileInfo).cast::<FileInfo>()) }
    }
}
